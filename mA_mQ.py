...
m0Q   = np.array([ma.cos(0.1)*ma.sin(2.2),ma.sin(0.1)*ma.sin(2.2),ma.cos(2.2)])
...
def Heff(m, Ms, N, K1vec, K1,alpha,gamma,V,T,dt,mn,d,A,r):
 ...
 return Huni(m, K1vec, Ms, K1) + Hdem(m, N, Ms) + Htherm(T,alpha,gamma,V,Ms,dt) + Hexch(m,mn,d,A,Ms) + Hstray(mn,r,Ms,V) 
...
write_data('dd_uu_sd_jm_300K_'+str(time.strftime("%Y%d%m%H%M%S", time.localtime()))+'.crv',create_header(),1.e-12,t,sol)
##########################################################################################
